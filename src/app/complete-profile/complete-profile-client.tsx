...
export default function CompleteProfileClient() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {   
      ...
      router.push(role === "landlord" ? "/verify-landlord-id" : "/home");
    } catch (error) {
          ...
          <form className="space-y-4" onSubmit={handleSubmit}>
            <div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <button type="button" onClick={() => setRole("student")} className={`relative rounded-lg p-4 text-left transition-all border-2 ${role === "student" ? "border-[#1a3c34] bg-white shadow-sm" : "border-gray-200 bg-white hover:border-gray-300 hover:shadow-sm"}`}>
                  <div className={`w-10 h-10 rounded-lg flex items-center justify-center mb-3 ${role === "student" ? "bg-[#1a3c34]" : "bg-gray-100"}`}>
                ...
                <button type="button" onClick={() => setRole("landlord")} className={`relative rounded-lg p-4 text-left transition-all border-2 ${role === "landlord" ? "border-[#1a3c34] bg-white shadow-sm" : "border-gray-200 bg-white hover:border-gray-300 hover:shadow-sm"}`}>
                  <div className={`w-10 h-10 rounded-lg flex items-center justify-center mb-3 ${role === "landlord" ? "bg-[#1a3c34]" : "bg-gray-100"}`}>
            ...
            <button type="submit" disabled={isLoading} className="w-full h-10 bg-[#1a3c34] hover:bg-[#15332c] disabled:bg-[#1a3c34]/70 text-white font-semibold text-sm rounded-lg transition-colors flex items-center justify-center gap-2">
